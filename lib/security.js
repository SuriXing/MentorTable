...
function enforceRateLimit(req, res, options = {}) {
  ...
  if (bucket.tokens < 1) {
    ...
    // F170 (P26): denials were silent — an operator watching Logs had no
...
function enforceLlmBreaker(req, res) {
  ...
  // F170 (P26): a tripped breaker is the cost-ceiling control actually
...
// ---------------------------------------------------------------------------
// PII redaction for structured logs (U8.1)
// ---------------------------------------------------------------------------
//
...
// F59 (U8.1 R2): extended key list. Adds password/secret/credential, the
...
function redactPII(value, seen) {
  ...
  // F60 (U8.1 R2): scan string values for secret-shaped substrings too —
...
// ---------------------------------------------------------------------------
// F166 (P22) — Global rate limiting via Vercel KV / Upstash REST
// ---------------------------------------------------------------------------
//
...
async function applyApiSecurity(req, res, options = {}) {
  ...
  // F166 (P22): global KV counter first when configured — it is the real
