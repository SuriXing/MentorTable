 ...
 *   catches naive flooding but NOT a distributed attack. See below for
 *   the global circuit breaker + LLM_DISABLED kill switch added in R2
...
function resolveAllowOrigin(reqOrigin) {
  ...
  if (list.length === 0) {
    ...
    if (process.env.VERCEL_ENV === 'production' || process.env.NODE_ENV === 'production') {
      console.warn(
        '[security] ALLOWED_ORIGINS is empty in production — withholding CORS headers entirely. Set ALLOWED_ORIGINS to your deployment URL if cross-origin access is intended.'
      );
      // Echoing the request origin would be a per-request wildcard, so send
      // no ACAO header at all: same-origin traffic needs no CORS, and
      // cross-origin reads fail in the browser.
      return '';
    }
...
// Vercel trust boundary for x-forwarded-for.
// -----------------------------------------------------------------------
...
//
// The FIRST entry is whatever the client sent in its own xff header, so a
// hostile client can rotate it to evade per-IP rate limiting. The LAST entry
// is written by the trusted edge, so that is what we key on.
function getClientIp(req) {
  const fwd = (req.headers && req.headers['x-forwarded-for']) || '';
  const chain = String(fwd).split(',').map((entry) => entry.trim()).filter(Boolean);
...
function enforceRateLimit(req, res, options = {}) {
  ...
  // Evict when the map is over budget. Buckets are re-inserted on every
  // admission (see below), so the front of the Map's iteration order really
  // is the least recently ADMITTED — an IP flood of denied requests does
  // not keep its buckets warm, so cold attacker buckets are evicted first.
  ...
  if (bucket.tokens < 1) {
    ...
    // : denials were silent — an operator watching Logs had no
    // signal that limiting was active. No IP in the event (PII discipline);
    // the bucket key prefix is enough to correlate.
    log('warn', 'rate_limited', { limiter: 'memory', bucketKeyPrefix: String(key).slice(0, 12), retryAfterSeconds: retryAfter });
    sendJsonError(res, 429, { error: 'Rate limit exceeded — please slow down.' });
    // Deliberately NO LRU touch on denial: a denied bucket keeps aging
    // toward eviction, so flooding from one IP does not pin its bucket in
    // the map while legitimate IPs get squeezed out.
    return false;
  }

  // Admission = use. Re-inserting moves the bucket to the back of the Map's
  // insertion order, which is what makes the front-of-map eviction above
  // true LRU instead of "oldest insertion".
...
// ---------------------------------------------------------------------------
// — Global LLM cost ceiling (autoscale circuit breaker + kill switch)
// ---------------------------------------------------------------------------
//
...
function getLlmHourlyTokenBudget() {
  const env = Number(process.env.LLM_TOKEN_HOURLY_BUDGET);
  if (Number.isFinite(env) && env > 0) return Math.floor(env);
  return 1000000; // default per-instance rolling-hour token ceiling
}
...
function checkLlmCircuitBreaker() {
  ...
  const tokenBudget = getLlmHourlyTokenBudget();
  if (_llmTokenCount >= tokenBudget) {
    const retryAfter = Math.max(
      1,
      Math.ceil((LLM_CALL_WINDOW_MS - (now - _llmCallWindowStart)) / 1000)
    );
    return {
      allowed: false,
      reason: 'LLM hourly token budget exceeded — service temporarily unavailable',
      status: 503,
      retryAfter,
    };
  }
...
/**
 * Meter actual upstream consumption (usage.total_tokens across providers)
 * into the same rolling window. Token budget trips the breaker for
 * subsequent requests once the accumulated usage crosses the ceiling, so a
 * few huge requests cannot hide behind the call-count budget.
 */
function recordLlmTokens(totalTokens) {
  const n = Number(totalTokens);
  if (!Number.isFinite(n) || n <= 0) return;
  const now = Date.now();
  _rollLlmWindowIfStale(now);
  _llmTokenCount += Math.floor(n);
}
...
function enforceLlmBreaker(req, res) {
  ...
  // : a tripped breaker is the cost-ceiling control actually
  // firing — it must be loud in Logs.
  log('warn', 'llm_breaker_blocked', { retryAfterSeconds: verdict.retryAfter, status: verdict.status });
...
function redactSensitive(str) {
  ...
  // 10. Aliyun RAM access keys: LTAI<base62>{12,24}. previous redactor
...
//
// : the function used to be named `sanitizeMentorField` which
...
// ---------------------------------------------------------------------------
//
// redactSensitive above is a STRING redactor for error previews — it scrubs
// secret-shaped substrings (sk-, Bearer, AWS access keys, JWTs, etc.) before
// they reach the client. redactPII below is a deep-OBJECT redactor for
// structured logger fields: it walks an object/array tree and replaces any
// value whose KEY matches a PII pattern with the sentinel string
// `"[redacted]"`. Handlers call this before handing request/user-shaped data
// to lib/logger.js so logs never carry raw emails, IPs, tokens, auth
// headers, cookies, or user identifiers.

// : extended key list. Adds password/secret/credential, the
// IP-leak header family (x-forwarded-for, forwarded, remoteAddress), session
// identifiers, and the bare `bearer` form some upstreams use as a key name.
const PII_KEY_RE = /email|ip|uid|user_id|phone|token|api[_-]?key|authorization|cookie|password|secret|credential|x-forwarded-for|forwarded|remoteAddress|session|sessionId|session_id|bearer/i;

function redactPII(value, seen) {
  if (value === null || value === undefined) return value;
  // : scan string values for secret-shaped substrings too —
  // the key-only walk missed Bearer/sk-…/LTAI… smuggled inside free-text
  // fields like `errorMessageTruncated` or `bodyTruncated`.
  if (typeof value === 'string') return redactSensitive(value);
  if (typeof value !== 'object') return value;
  // Guard against circular references — return a sentinel rather than
  // recursing forever, so logger callers never blow the stack.
  const tracker = seen || new WeakSet();
  if (tracker.has(value)) return '[circular]';
  tracker.add(value);
  if (Array.isArray(value)) {
    return value.map((item) => redactPII(item, tracker));
  }
  const out = {};
  for (const key of Object.keys(value)) {
    if (PII_KEY_RE.test(key)) {
      ...
      out[key] = redactPII(value[key], tracker);
    }
  }
  return out;
}
...
// ---------------------------------------------------------------------------
// — Global rate limiting via Vercel KV / Upstash REST
// ---------------------------------------------------------------------------
//
// The in-memory bucket above only protects within one warm serverless
// instance. When KV_REST_API_URL / KV_REST_API_TOKEN (Vercel KV) or
// UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN are configured, this
// fixed-window counter runs on shared state and every instance sees the
// same budget. Implemented with plain REST (INCR + EXPIRE NX pipeline) so
// no SDK dependency is required.
//
// Semantics: fixed window (not token bucket) — allows a 2x burst across a
// window boundary. Acceptable for a global cap whose job is cost survival,
// not fairness; the per-instance token bucket still smooths bursts.

function kvRateLimitEnv() {
  const url = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL || '';
  const token = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN || '';
  return url && token ? { url: url.replace(/\/$/, ''), token } : null;
}

let _kvRateLimitFetch = global.fetch; // injectable for tests

/**
 * Global fixed-window counter on KV. Returns:
 *   true  — allowed (count within limit)
 *   false — blocked (429 already sent)
 *   null  — KV not configured / call failed → caller falls back to memory
 */
async function checkKvRateLimit(req, res, options = {}) {
  const env = kvRateLimitEnv();
  if (!env || typeof _kvRateLimitFetch !== 'function') return null;
  const limit = Number(options.kvLimit || 0);
  if (!(limit > 0)) return null;
  const windowSeconds = Number(options.kvWindowSeconds || 60);
  const key = `ratelimit:${options.bucketName || 'api'}:${getClientIp(req)}`;
  try {
    const resp = await _kvRateLimitFetch(`${env.url}/pipeline`, {
      method: 'POST',
    ...
    const count = Number(first ? first.result : undefined);
    if (!Number.isFinite(count)) return null;
    if (count > limit) {
      const retryAfter = Math.max(1, windowSeconds);
      res.setHeader('Retry-After', String(retryAfter));
      log('warn', 'rate_limited', { limiter: 'kv', bucketKeyPrefix: String(key).slice(0, 24), retryAfterSeconds: retryAfter, count, limit });
      sendJsonError(res, 429, { error: 'Rate limit exceeded — please slow down.' });
      return false;
    }
    return true;
  } catch {
    // KV unreachable — degrade to the in-memory bucket rather than failing
    // open or closing the endpoint entirely.
    return null;
  }
}

// Test helper: point the KV limiter at a stubbed fetch.
function _setKvRateLimitFetch(fn) {
  _kvRateLimitFetch = fn;
}
 ...
 */
async function applyApiSecurity(req, res, options = {}) {
  ...
  if (rateLimitDisabled || options.rateLimit === false) return true;
  // : global KV counter first when configured — it is the real
  // cross-instance budget. Unconfigured / errored KV falls back to the
  // per-instance token bucket below.
  if (options.rateLimit && typeof options.rateLimit.kvLimit === 'number') {
    const kvResult = await checkKvRateLimit(req, res, {
      ...options.rateLimit,
      bucketName: options.rateLimit.bucketName || (options.maxBodyBytes ? 'table' : 'api'),
    });
