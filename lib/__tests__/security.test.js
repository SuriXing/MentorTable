...
describe('enforceRateLimit', () => {
  ...
  it('keys on the LAST (edge-provided) IP in X-Forwarded-For chain', () => {
    // The client can claim anything in the leading entries; the edge writes
    // the final one. Rotation of the claimed entry must NOT evade the bucket.
    const res = mockRes();
    expect(enforceRateLimit(
      mockReq({ headers: { 'x-forwarded-for': '10.9.9.9, 10.0.0.3' } }),
      res,
      { capacity: 1, refillPerSecond: 0 }
    )).toBe(true);
    expect(enforceRateLimit(
      mockReq({ headers: { 'x-forwarded-for': '10.8.8.8, 10.0.0.3' } }),
      mockRes(),
      { capacity: 1, refillPerSecond: 0 }
    )).toBe(false);
  });
...
describe('getClientIp', () => {
  it('takes the edge-written LAST entry of the xff chain', () => {
    expect(getClientIp(mockReq({ headers: { 'x-forwarded-for': '1.2.3.4, 5.6.7.8' } }))).toBe('5.6.7.8');
    expect(getClientIp(mockReq({ headers: { 'x-forwarded-for': '1.2.3.4' } }))).toBe('1.2.3.4');
  });
...
describe('applyApiSecurity', () => {
  ...
  it('KV outage degrades to memory-bucket limiting, never fail-open', async () => {
...
describe('LLM token-budget breaker', () => {
  const savedEnv = {};
  const envKeys = ['LLM_DISABLED', 'LLM_HOURLY_BUDGET', 'LLM_TOKEN_HOURLY_BUDGET'];
  beforeEach(() => {
  ...
  it('stays open while accumulated tokens are under the ceiling', () => {
    process.env.LLM_TOKEN_HOURLY_BUDGET = '1000';
    _resetLlmCircuitBreaker();
    expect(checkLlmCircuitBreaker().allowed).toBe(true);
    recordLlmTokens(900);
    expect(checkLlmCircuitBreaker().allowed).toBe(true);
  ...
  it('trips on accumulated token usage even when the call count is tiny', () => {
    process.env.LLM_TOKEN_HOURLY_BUDGET = '1000';
    _resetLlmCircuitBreaker();
    recordLlmTokens(400);
    recordLlmTokens(700);
    ...
    expect(verdict.reason).toContain('token budget');
  });

  it('ignores non-positive or malformed token reports', () => {
    process.env.LLM_TOKEN_HOURLY_BUDGET = '1000';
    _resetLlmCircuitBreaker();
    recordLlmTokens(0);
    recordLlmTokens(-5);
    recordLlmTokens('not-a-number');
    recordLlmTokens(undefined);
    expect(checkLlmCircuitBreaker().allowed).toBe(true);
  });
});

describe('production CORS with empty allowlist withholds ACAO entirely', () => {
  const savedEnv = {};
  const envKeys = ['ALLOWED_ORIGINS', 'NODE_ENV', 'VERCEL_ENV'];
  beforeEach(() => {
    for (const key of envKeys) {
      savedEnv[key] = process.env[key];
      delete process.env[key];
  ...
  it('does not echo an attacker origin in production (no ACAO header)', () => {
    process.env.NODE_ENV = 'production';
    delete process.env.ALLOWED_ORIGINS;
    expect(resolveAllowOrigin('https://attacker.com')).toBe('');
    const res = mockRes();
    applyCorsHeaders(mockReq({ headers: { origin: 'https://attacker.com' } }), res);
    expect(res._headers['Access-Control-Allow-Origin']).toBeUndefined();
  });

  it('same-origin requests need no ACAO and dev wildcard behavior is unchanged', () => {
    delete process.env.ALLOWED_ORIGINS;
    delete process.env.NODE_ENV;
    expect(resolveAllowOrigin('https://anything.dev')).toBe('*');
  });

  it('explicit allowlist still works in production', () => {
    process.env.NODE_ENV = 'production';
    process.env.ALLOWED_ORIGINS = 'https://mentor-table.vercel.app';
    expect(resolveAllowOrigin('https://mentor-table.vercel.app')).toBe('https://mentor-table.vercel.app');
  });
});
